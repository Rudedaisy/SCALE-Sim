...
for i in range(len(PENNI2FileNames)):
    files.append(paths[3] + PENNI3FileNames[i])
for i in range(len(PENNI2FileNames)):
...
stdPlotLegend = ['Baseline','2Stage','PENNI','PENNI+2Stage','PENNI+2Stage+ZeroSkip','+chunk_regularize'] #, '+dynamic_bounds']
...
def plotCycleUtil():
    ...
    ax.legend(['2Stage','PENNI+2Stage','PENNI+2Stage+ZeroSkip', '+chunk_regularize', 'Best case'], loc=2)
    ax.grid(True)
    plt.show()
    ...
    ax.legend(['baseline','Decouple','Sq+Dc','Sq+Dc+aa_coeff_sq','+chunk_regularize'], loc=3)
    ax.grid(True)
    plt.show()
